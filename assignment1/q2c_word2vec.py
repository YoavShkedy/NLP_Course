...
def naive_softmax_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    # Compute the predictions (softmax)
    ...
    y_hat[outside_word_idx] -= 1  # Subtract 1 from the true outside word
...
def neg_sampling_loss_and_gradient(
    ...
    # Compute the loss and gradient for the true outside word
    u_o = outside_vectors[outside_word_idx]
    z = sigmoid(np.dot(u_o, center_word_vec))
    loss = -np.log(z)
    grad_center_vec += (z - 1) * u_o
    grad_outside_vecs[outside_word_idx] += (z - 1) * center_word_vec

    # Compute the loss and gradients for the negative samples
    for k in neg_sample_word_indices:
        u_k = outside_vectors[k]
        z = sigmoid(-np.dot(u_k, center_word_vec))
        loss -= np.log(z)
        grad_center_vec += (1 - z) * u_k
        grad_outside_vecs[k] += (1 - z) * center_word_vec
    ### END YOUR CODE
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    for outside_word in outside_words:
        ...
        curr_loss, curr_grad_center, curr_grad_outside = word2vec_loss_and_gradient(
            center_word_vec, outside_word_idx, outside_vectors, dataset
        )
        loss += curr_loss
        grad_center_vecs[center_word_idx] += curr_grad_center
        grad_outside_vectors += curr_grad_outside
    ### END YOUR CODE
