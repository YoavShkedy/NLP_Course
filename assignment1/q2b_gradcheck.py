...
def gradcheck_naive(f, x, gradient_text=""):
    ...
    while not it.finished:
        ...
        ### YOUR CODE HERE:
        # Save the current value of x[ix]
        old_value = x[ix]
...
def your_gradcheck_test():
    ...
    # Example: Check a more complex function
    cubic = lambda x: (np.sum(x ** 3), 3 * x ** 2)
    gradcheck_naive(cubic, np.array(123.456), "Cubic function test - scalar")
    gradcheck_naive(cubic, np.random.randn(3, ), "Cubic function test - 1D")
    gradcheck_naive(cubic, np.random.randn(4, 5), "Cubic function test - 2D")
    ### END YOUR CODE
